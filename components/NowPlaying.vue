...
import { IconLink, IconMusic, IconPlayerPlay, IconPlayerPause, IconVolume } from '@tabler/icons-vue';

// Interface for Spotify API response
...
const localProgress = ref(0); // Track progress locally between API calls
const lastFetchTime = ref(Date.now()); // When we last fetched data
...
async function fetchNowPlaying() {
  try {
    ...
    // Initialize local progress with the latest API value
    if (nowPlaying.value) {
...
// Progress updater (runs every 1000ms / 1 second)
...
function startProgressUpdater() {
  ...
  progressInterval = setInterval(() => {
    if (nowPlaying.value && nowPlaying.value.isPlaying) {
      // Calculate time elapsed since last API fetch
      const elapsed = Date.now() - lastFetchTime.value;
      
      // Update local progress (original progress + elapsed time)
      localProgress.value = Math.min(
        nowPlaying.value.progress + elapsed,
        nowPlaying.value.duration
      );
      
      // If we reached the end of song, fetch new data
...
onMounted(() => {
  ...
  refreshInterval = setInterval(() => {
    fetchNowPlaying();
  }, 30000); // Refresh from API every f30 seconds
});
...
<template>
  <section class="flex flex-col gap-3">
    ...
    <UCard
      ...
      :ui="{
        body: { padding: 'p-0' },
        background: 'bg-white dark:bg-primary-950 border border-stone-200 dark:border-stone-600',
        ring: ''
      }"
    >
      ...
      <template v-else-if="error">
        <div class="flex flex-col items-center justify-center p-6 gap-2">
          ...
          <UButton size="sm" @click="fetchNowPlaying" variant="soft" class="mt-2">
      ...
      <template v-else-if="nowPlaying && !nowPlaying.isPlaying">
        <div class="flex flex-col items-center justify-center p-6 gap-2">
          <IconVolume class="text-neutral-400 w-10 h-10" />
          <p class="text-sm text-neutral-500">{{ $t('not_playing', 'Ababil Not playing anything right now') }}</p>
        </div>
      </template>
      ...
      <template v-else-if="nowPlaying">
          ...
          <div class="flex flex-col justify-between p-3 sm:p-4 flex-1">
            <div>
              <div class="flex items-center gap-2">
                <IconPlayerPlay v-if="nowPlaying.isPlaying" class="w-4 h-4 text-primary-500 animate-pulse" />
            ...
            <!-- Progress bar -->
            <div class="mt-2">
              <div class="h-1 w-full bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                <div 
        ...
        <!-- Progress animation overlay - shows refresh timing -->
        <div 
...
.music-bars span {
  display: block;
  width: 3px;
  background: linear-gradient(to top, theme('colors.primary.500'), theme('colors.primary.300'));
...
/* Progress animation for refresh indicator */
