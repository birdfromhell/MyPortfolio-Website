...
<template>
  ...
  >
    ...
    >
      <p
        class="text-xs text-neutral-600 dark:text-neutral-400 w-full sm:w-1/2 line-clamp-4 sm:line-clamp-none"
      >
        {{ getLocalizedContent }}
      </p>
      <img
    ...
    <div class="mt-4 flex flex-row items-center justify-start gap-2 w-full">
      ...
      <UButton @click="previewModalOpen = true" variant="solid">
      ...
      <div>
        <div
          v-if="
            ...
            (currentPreviewMode === 'live' || !hasVideoPreview(project))
          "
          class="text-center"
        >
          ...
          >
            <IconPlayerPlay class="w-4 h-4 mr-1" />
            {{ $t("view", "Open Live Project") }}
          </UButton>
        </div>
        <div
          v-else-if="
            hasVideoPreview(project) &&
            (currentPreviewMode === 'video' || !hasLivePreview(project))
          "
          class="text-center"
        >
          <p class="mb-4">
            Watch the video preview because live preview not possible 😁
          </p>
          <div
            class="aspect-video w-full mt-2 mb-4"
            v-html="getSafeVideoEmbed(project.video_url)"
          ></div>
        </div>
        <div v-else class="text-center py-4">
          <template v-if="isClosedSource(project)">
            <div class="closed-source-container relative">
              <div
                class="absolute inset-0 bg-gradient-to-r from-amber-50/0 via-amber-100/10 to-amber-50/0 animate-shimmer"
              ></div>
              <div class="relative z-10">
                <IconLock
                  class="text-4xl text-amber-500 mb-3 animate-pulse-slow mx-auto"
                />
                <h4
              ...
              <p class="mb-5 text-neutral-600 dark:text-neutral-400">
                Oops! I was too lazy to upload a preview for this project. And
                the code? It's closed source because... well, reasons. 🤷‍♂️
              </p>
              <!-- Casual explanation box -->
              <div
                class="bg-amber-50 dark:bg-amber-900/30 p-4 rounded-md mb-5 text-left border-l-4 border-amber-500"
              >
                <p class="text-sm leading-relaxed">
                  <span class="font-bold">The real deal:</span> This project is
                  closed source, which means I can't share the code publicly. It
                  might be client work, contain proprietary magic, or maybe it's
                  just hiding my terrible variable names. Either way, it's under
                  digital lock and key!
                </p>
              </div>
              <p
                class="text-xs text-amber-600 dark:text-amber-400 italic mb-5 px-4 py-2 bg-amber-50/50 dark:bg-amber-900/20 rounded-md inline-block"
              >
          ...
          <template v-else>
            <IconCode
              class="text-3xl text-primary-500 mb-3 animate-pulse-slow mx-auto"
            />
            <h4 class="font-bold text-lg mb-2">Do-It-Yourself Preview!</h4>
            <p class="mb-5 text-neutral-600 dark:text-neutral-400">
              No preview available? No problem! Clone this project and run it
              yourself - it's like IKEA furniture, but with code! 🛠️
            </p>
            <div
              class="bg-neutral-100 dark:bg-primary-900/50 p-4 rounded-md mb-5 overflow-x-auto"
            >
              <pre
                class="text-xs text-left"
              ><code>git clone {{ project.repo_link }}
...
# Or just stare at the code until it makes sense 🧙‍♂️</code></pre>
            ...
            <p class="text-xs text-neutral-500 italic mb-5">
              "The best preview is the one you build yourself" - Ancient
              developer proverb
            </p>
            <UButton
              :to="getRepoLink(project)"
